...
| **2. Entities**            | **Sometimes** — if a clear Strategy seam exists, declare the interface as an entity. | E.g., `SpotLookupStrategy` in Parking Lot, `DispatchStrategy` in Elevator. For Amazon Locker, no such seam is mandatory — allocation is "first matching size", and that's it. |
| **3. Class Design**        | **YES, when the pattern earns rent in the base.** Name it explicitly.               | India-based interviews expect candidates to name patterns when applied. Don't artificially defer. For Amazon Locker specifically, the base doesn't need a pattern — and that's OK. |
...
> **Amazon Locker specifically:** the base design genuinely doesn't *need* a pattern — encapsulation + Information Expert + DI of Clock/Random do the work. Allocation is "first matching free compartment"; if the interviewer pushes "size fallback" or "different code generators" *then* Strategy lands in Step 5. Your senior one-liner at the end of Step 3:
> *"The base design relies on Information Expert, Tell-Don't-Ask, and DI — no GoF pattern earns rent here yet. When extensibility prompts come (size fallback, two-phase reserve/confirm, notifications), I'll point out where Strategy, Command, and Observer would land."*
>
> Contrast with Parking Lot / Elevator where `SpotLookupStrategy` / `DispatchStrategy` ARE in the base — those problems have an obviously pluggable policy from the start; this one doesn't.
...
mvn -q compile exec:java \
  -Dexec.mainClass=com.conceptcoding.interviewquestions.hello_all_questions.amazonlocker.LockerDriver
```
